...
function NavItem({ icon, label, active = false, onClick }: { icon: any, label: string, active?: boolean, onClick?: () => void }) {
    return (
        <div 
            onClick={onClick}
            className={`flex items-center gap-4 px-4 py-3 rounded-xl cursor-pointer transition-colors ${active ? 'bg-[#FCD34D] font-bold text-black' : 'text-gray-500 hover:bg-gray-50'}`}
        >
            {icon}
            <span>{label}</span>
        </div>
...
function StatCard({ title, value }: any) {
    return (
        <div className="bg-[#F4A261] p-6 rounded-2xl text-white shadow-sm flex flex-col justify-between h-32 relative overflow-hidden group">
    ...
    return (
        <div className="flex items-center justify-between p-2 hover:bg-gray-50 rounded-lg transition-colors cursor-pointer">
            <div className="flex items-center gap-4">
                <div className="h-12 w-12 bg-[#FCD34D] rounded-xl flex items-center justify-center font-bold text-lg">{id}</div>
                <div>
                    <div className="font-bold flex items-center gap-2">{name} <span className="text-[10px] bg-green-100 text-green-600 px-2 py-0.5 rounded-full uppercase tracking-wide font-extrabold">{status}</span></div>
